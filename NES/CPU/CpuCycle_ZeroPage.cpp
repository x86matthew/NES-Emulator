...
DWORD HandleInstructionCycle_ZeroPage()
{
	BYTE bValue = 0;
	BYTE bZeroPageOffset = 0;

	if(gSystem.Cpu.InstructionState == INSTRUCTION_STATE_ZERO_PAGE_BEGIN)
	{
		// read byte
		...
		{
			// read instruction
			gSystem.Cpu.InstructionState = INSTRUCTION_STATE_ZERO_PAGE_READ_ADDRESS;
		}
		else if(gSystem.Cpu.pInstructionAttributes->bFlags & INSTRUCTION_FLAGS_WRITE)
		{
			// write instruction
			gSystem.Cpu.InstructionState = INSTRUCTION_STATE_ZERO_PAGE_WRITE_ADDRESS;
		}
	...
	else if(gSystem.Cpu.InstructionState == INSTRUCTION_STATE_ZERO_PAGE_READ_ADDRESS)
	{
		// read value
		...
		else if(gSystem.Cpu.pInstructionAttributes->bFlags & INSTRUCTION_FLAGS_READ_MODIFY_WRITE)
		{
			...
			gSystem.Cpu.InstructionState = INSTRUCTION_STATE_ZERO_PAGE_MODIFY;
		}
		else
		{
			return 1;
	...
	else if(gSystem.Cpu.InstructionState == INSTRUCTION_STATE_ZERO_PAGE_MODIFY)
	{
	...
	else if(gSystem.Cpu.InstructionState == INSTRUCTION_STATE_ZERO_PAGE_WRITE_ADDRESS)
	{
